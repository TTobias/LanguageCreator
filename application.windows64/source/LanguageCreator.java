...
public class LanguageData{
  ...
  public void loadFromFile(){
    ...
    for(int i = 1; i<PApplet.parseInt(lodWords[0]); i++){
      wordlist.add(new WordTranslation(split(lodWords[i]," : ")[0],split(lodWords[i]," : ")[1],PApplet.parseInt(split(lodWords[i]," : ")[2])) );
    }
...
public class UI_VocabularyList extends UIObject{
  ...
  public UI_VocabularyList(Vector2 pos, Vector2 exp, UI_InputKeyboard k){ 
    ...
    scrollwheel = new UI_Scrollwheel(new Vector2(position.x+expanse.x*0.97f, position.y+expanse.y*0.095f), new Vector2(expanse.x*0.03f, expanse.y*0.855f) ,true, false);
...
public class UI_Scrollwheel extends UIObject{
  ...
  public void show(){
    ...
    if(onMove){
      ...
      rect(position.x,position.y+ (expanse.y*0.9f * tmpState),expanse.x,expanse.y*0.1f);
    }else{
      stroke(0);
      fill(ColorCode.grey);
      rect(position.x,position.y+ (expanse.y*0.9f * state),expanse.x,expanse.y*0.1f);
    }
  }
  
  public void onMouseDown(){
    if(active && mouseButton == LEFT && !onMove && mouseX > position.x && mouseX < position.x+expanse.x && mouseY > position.y+ (expanse.y*0.9f*state) && mouseY < position.y+ (expanse.y*state)){
      onMove = true;
      tmpMouse.setToMouse();
  ...
  public void onMouse(){
    if(active && onMove){
      ...
      float posy = position.y + expanse.y*0.9f+state + shift;
      tmpState = position.y;
...
public class UI_QuantifierList extends UIObject{
  ...
  public void showListObject(float posx, float posy, float expx, float expy, int index){
    ...
    fill(ColorCode.guiText);
    textSize(expy*0.3f);
    text( list.get(index).name, posx+expx*0.02f,posy+expy*0.33f);
    text( list.get(index).toString(), posx+expx*0.02f,posy+expy*0.78f);
  }
...
public class UI_QuantifierEditor extends UIObject{
  ...
  public UI_QuantifierEditor(Vector2 pos, Vector2 exp, UI_InputKeyboard k){ 
    ...
    wordTypeSelection = new UI_SwitchcaseButton(new Vector2(position.x+3,position.y+3), new Vector2(expanse.x*0.15f, expanse.y-6));
    for(int i = 0; i<QuantifierType.getAmount(); i++){
      wordTypeSelection.addButton( new UI_Button(new Vector2(position.x+8,position.y+8+expanse.y*0.05f*i), new Vector2(expanse.x*0.15f-16,expanse.y*0.045f), QuantifierType.toString(i) ,true) );
    }
    
    modificationSelection = new UI_SwitchcaseButton(new Vector2(position.x+10+expanse.x*0.15f,position.y+5), new Vector2(expanse.x*0.82f, expanse.y*0.12f));
    modificationSelection.addButton( new UI_Button(new Vector2(position.x+15+expanse.x*0.2f,position.y+12), new Vector2(expanse.x*0.22f,expanse.y*0.045f), " Pre-Syllable" ,true) );
    modificationSelection.addButton( new UI_Button(new Vector2(position.x+15+expanse.x*0.45f,position.y+12), new Vector2(expanse.x*0.22f,expanse.y*0.045f), " Post-Syllable" ,true) );
    modificationSelection.addButton( new UI_Button(new Vector2(position.x+15+expanse.x*0.7f,position.y+12), new Vector2(expanse.x*0.22f,expanse.y*0.045f), " In-Syllable" ,true) );
    modificationSelection.addButton( new UI_Button(new Vector2(position.x+15+expanse.x*0.2f,position.y+12+expanse.y*0.05f), new Vector2(expanse.x*0.22f,expanse.y*0.045f), " Part Duplication" ,true) );
    modificationSelection.addButton( new UI_Button(new Vector2(position.x+15+expanse.x*0.45f,position.y+12+expanse.y*0.05f), new Vector2(expanse.x*0.22f,expanse.y*0.045f), " Replacement" ,true) );
    modificationSelection.addButton( new UI_Button(new Vector2(position.x+15+expanse.x*0.7f,position.y+12+expanse.y*0.05f), new Vector2(expanse.x*0.22f,expanse.y*0.045f), " Modification" ,true) );
    
    deleteButton = new UI_Button(new Vector2(position.x+expanse.x*0.8f, position.y+expanse.y*0.92f),new Vector2(expanse.x*0.15f,expanse.y*0.045f), " Delete",true);
    applyButton = new UI_Button(new Vector2(position.x+expanse.x*0.6f, position.y+expanse.y*0.92f),new Vector2(expanse.x*0.15f,expanse.y*0.045f), " Apply",true);
    ...
    previewText = new UI_Text(new Vector2(position.x+expanse.x*0.2f,position.y+expanse.y*0.25f), new Vector2(expanse.x*0.75f, expanse.y*0.12f), "PREVIEW");
    onsetFixedCheckbox = new UI_Checkbox(new Vector2(position.x+expanse.x*0.2f,position.y+expanse.y*0.45f), new Vector2(expanse.x*0.22f, expanse.y*0.04f), "Fixed Onset?", true);
    nucleusFixedCheckbox = new UI_Checkbox(new Vector2(position.x+expanse.x*0.45f,position.y+expanse.y*0.45f), new Vector2(expanse.x*0.22f, expanse.y*0.04f), "Fixed Nucleus?", true);
    codaFixedCheckbox = new UI_Checkbox(new Vector2(position.x+expanse.x*0.7f,position.y+expanse.y*0.45f), new Vector2(expanse.x*0.22f, expanse.y*0.04f), "Fixed Coda?", true);
    onsetTextInput = new UI_TextInputField(new Vector2(position.x+expanse.x*0.25f,position.y+expanse.y*0.52f), new Vector2(expanse.x*0.1f, expanse.y*0.06f),"",keyboard,true);
    nucleusTextInput = new UI_TextInputField(new Vector2(position.x+expanse.x*0.5f,position.y+expanse.y*0.52f), new Vector2(expanse.x*0.1f, expanse.y*0.06f),"",keyboard,true);
    codaTextInput = new UI_TextInputField(new Vector2(position.x+expanse.x*0.75f,position.y+expanse.y*0.52f), new Vector2(expanse.x*0.1f, expanse.y*0.06f),"",keyboard,true);
    onsetPositionInput = new UI_IntegerField(new Vector2(position.x+expanse.x*0.2f,position.y+expanse.y*0.6f), new Vector2(expanse.x*0.24f, expanse.y*0.05f),"Onset",1,-8,8);
    nucleusPositionInput = new UI_IntegerField(new Vector2(position.x+expanse.x*0.45f,position.y+expanse.y*0.6f), new Vector2(expanse.x*0.24f, expanse.y*0.05f),"Nucleus",1,-8,8);
    codaPositionInput = new UI_IntegerField(new Vector2(position.x+expanse.x*0.7f,position.y+expanse.y*0.6f), new Vector2(expanse.x*0.24f, expanse.y*0.05f),"Coda",1,-8,8);
    onsetActiveCheckbox = new UI_Checkbox(new Vector2(position.x+expanse.x*0.22f,position.y+expanse.y*0.67f), new Vector2(expanse.x*0.15f, expanse.y*0.04f),"copy onset?",true);
    codaActiveCheckbox = new UI_Checkbox(new Vector2(position.x+expanse.x*0.72f,position.y+expanse.y*0.67f), new Vector2(expanse.x*0.15f, expanse.y*0.04f),"copy coda?",true);
    
    syllablePositionInput = new UI_IntegerField(new Vector2(position.x+expanse.x*0.2f,position.y+expanse.y*0.15f),new Vector2(expanse.x*0.75f,expanse.y*0.05f)," Syllable Position:",1,-8,8);
    duplicationStartInput = new UI_IntegerField(new Vector2(position.x+expanse.x*0.25f,position.y+expanse.y*0.25f),new Vector2(expanse.x*0.6f,expanse.y*0.05f)," Starting Syllable:",1,-8,8);
    duplicationEndInput =   new UI_IntegerField(new Vector2(position.x+expanse.x*0.25f,position.y+expanse.y*0.32f),new Vector2(expanse.x*0.6f,expanse.y*0.05f)," Ending Syllable:",1,-8,8);
    duplicateWithCodaCheckbox =  new UI_Checkbox(new Vector2(position.x+expanse.x*0.35f,position.y+expanse.y*0.45f),new Vector2(expanse.x*0.4f,expanse.y*0.05f),"keep First Coda?",true);
    duplicateWithOnsetCheckbox = new UI_Checkbox(new Vector2(position.x+expanse.x*0.35f,position.y+expanse.y*0.52f),new Vector2(expanse.x*0.4f,expanse.y*0.05f),"keep Last Onset?",true);
    duplicatePositionCheckbox = new UI_Checkbox(new Vector2(position.x+expanse.x*0.3f,position.y+expanse.y*0.62f),new Vector2(expanse.x*0.5f,expanse.y*0.05f),"copy to preposition?",true);
